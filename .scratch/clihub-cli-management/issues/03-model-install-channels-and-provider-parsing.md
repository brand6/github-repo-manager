...
- [ ] A custom online install command is parsed into structured provider metadata, terminal-only installer metadata, or rejected.
- [ ] Supported command parsing covers common npm, winget, choco, scoop, and trusted terminal-only installer-command shapes.
- [ ] Official PowerShell installer commands can be saved as terminal-only channels when the source is trusted.
- [ ] Complex shell, pipe, redirection, `cmd /c`, `powershell -Command`, `curl | sh`, and `iwr | iex` commands are rejected for hidden backend execution.
